...
- [x] **API 전역 큐와 쿨다운** 완료 (2026-09-04). 아래 "요청 제한" 절 참고.
      ...
      **배포본에서 실검증 완료** — 동시 4건 중 1건만 통과, 챌린지 6번째부터 429. 지금 `ChessComClient`의 직렬 큐는
      **인스턴스 필드**라 `new ChessComClient()`마다 따로 생긴다. 클래스 주석의
      "요청은 엄격히 하나씩"은 인스턴스 안에서만 참이다. 동시 사용자 N명 = 병렬 큐 N개.
      **주의**: 서버리스에서는 모듈 전역 큐도 진짜 전역이 아니다(인스턴스마다 따로).
      진짜 전역이려면 DB 기반 리스가 필요하다 — 단, 단일 VM 배포라면 불필요해진다.
      아래 배포 방식 결정에 달려 있다.
      함께 필요: 사용자별 쿨다운(`players.lastSyncedAt` 활용),
      정직한 User-Agent(현재 `local single-user app`이라 배포 시 거짓).
...
→ **이 결정 전까지 전역 큐 구현은 보류.** 방식에 따라 만들 것이 달라진다.

## 다음 할 일

- [x] 본인 확인 + 세션/소유권 모델 완료 (2026-09-03)
- [ ] **배포 방식 결정** (Vercel vs 오라클 VM) ← 전역 큐 설계가 여기에 달려 있음
- [ ] 전역 API 큐·쿨다운·어뷰즈 상한
