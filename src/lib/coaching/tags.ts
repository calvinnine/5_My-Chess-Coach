...
/**
 * Which part of the game a player would train to fix this.
 *
 * Deliberately separate from `category`: a back-rank problem is tactical in
 * nature but is trained as tactics, while a repertoire gap is also "opening"
 * but is not a tactic at all. `habit` holds the clock and decision problems that
 * belong to no phase — they are kept visible rather than folded into a phase
...
/** The game phase whose accuracy best reflects each perspective, if any. */
export const PERSPECTIVE_PHASE: Record<Perspective, "opening" | "middlegame" | "endgame" | null> = {
  opening: "opening",
  tactics: "middlegame",
  strategy: "middlegame",
  endgame: "endgame",
